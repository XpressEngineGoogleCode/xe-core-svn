...
class FileController extends file
{
	 ...
	 * @developer
	 * @access public
	 * @param $editor_sequence
	 * @param $upload_target_srl
	 * @return void
	 */
	public function setUploadInfo($editor_sequence, $upload_target_srl = 0)
	{
	...
	function insertFile($file_info, $module_srl, $upload_target_srl, $download_count = 0, $manual_insert = FALSE)
	{
		...
		if(preg_match("/\.(jpe?g|gif|png|wm[va]|mpe?g|avi|swf|flv|mp[1-4]|as[fx]|wav|midi?|moo?v|qt|r[am]{1,2}|m4v)$|ogg/i", $file_info['name']))
		{
	...
	public function moveFile($source_srl, $target_module_srl, $target_srl)
	{
		...
		for($i = 0 ; $i < $file_count ; $i++)
		{
			...
			if(preg_match("/\.(jpe?g|gif|png|wm[va]|mpe?g|avi|swf|flv|mp[1-4]|as[fx]|wav|midi?|moo?v|qt|r[am]{1,2}|m4v|ogg)$/i", $file_info->source_filename))
			{
