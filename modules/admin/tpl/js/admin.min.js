...
setCookie("__xe_admin_gnb_xe_status",g,f);for(var j in __xe_admin_gnb_txs)g=__xe_admin_gnb_txs[j],h=getCookie(g),setCookie(g,h,f)};a(".gnb").gnb();a(".x #lang").mouseleave(function(){a(this).hide()}).focusout(function(){var b=a(this);setTimeout(function(){b.find("a:focus").length||b.mouseleave()},500)});a(".x th :checkbox").change(function(){var b=a(this),e=b.data("name");b.closest("table").find(":checkbox").filter(function(){var b=a(this);return!b.prop("disabled")&&(b.attr("name")==e||b.data("name")==
e)}).prop("checked",b.prop("checked")).end().end().trigger("update.checkbox",[e,this.checked])});a(document.body).on("click",".x .x_pagination .x_disabled, .x .x_pagination .x_active",function(){return!1});if(1<a(".x .section").length){var h=a(".x .section").find(">h1:first");h.each(function(){var b=a(this);b.next().length&&b.append('<button type="button" class="snToggle x_icon-chevron-up">Toggle this section</button>')});a(".x .section.collapsed>h1>.snToggle").removeClass("x_icon-chevron-up").addClass("x_icon-chevron-down");
