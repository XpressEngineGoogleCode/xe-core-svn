...
jQuery(function($){
    ...
    suForm.find(':checkbox').each(function(){
        ...
        $i.change(function(){
            if($i.is(':checked')){
                $i.parent('td').next('td').find(':radio, :text').removeAttr('disabled').end().find(':radio[value=option]').attr('checked', 'checked');
                $i.parent('td').next('td').find('label').removeClass('disabled');
                //$i.parent('td').next('td').find('input[type=radio]:checked').next('label').addClass('checked');
            } else {
                $i.parent('td').next('td').find(':radio, :text').attr('disabled','disabled').removeAttr('checked');
                $i.parent('td').next('td').find('label').addClass('disabled');
            }
        });
    });
    ...
    $('._addDeniedID').click(function(){
        ...
        jQuery.exec_json('member.procMemberAdminInsertDeniedID', {
            'user_id': ids
        }, on_complete);
