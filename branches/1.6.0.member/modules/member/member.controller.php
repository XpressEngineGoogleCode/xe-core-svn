<?php
    ...
    class memberController extends member {
		 ...
		 */
		public function procMemberDriverInterface()
		{
		 ...
		 * @developer NHN (developers@xpressengine.com)
		 */
		public function procMemberSignin()
		{
			...
			if(!in_array($driverName, $config->usedDriver))
			{
				return $this->stop('msg_invalid_request');
			}

			$loginInfo = Context::getRequestVars();
			$driver = getDriver('member', $driverName);

			try
			{
				$this->memberVo = $driver->validateLoginInfo($loginInfo);
			}
		...
		/**
		 * @brief SignIn by membersrl
		 * @return stdClass (리턴타입이 null이라면 로그인 실패)
		 * @developer NHN (developers@xpressengine.com)
		 */
		public function doSignin($driverName, $memberSrl)
		{
			...
			try
			{
				$output = $oDriver->doSignin($memberSrl);
			}
			catch(MemberDriverException $e)
			{
				if($e->getCode() == MemberDriverException::REDIRECT)
				{
			...
			$trigger_output = ModuleHandler::triggerCall('member.doSignin', 'after', $this->memberInfo);
		...
		public function setSessionInfo()
		{
			...
			$args->member_srl = ($this->memberInfo) ? $this->memberInfo->member_srl : $_SESSION['member_srl'];
		...
		function procMemberModifyInfo()
		{
			...
			// If your information came through the current session information to extract information from the users
			$args = new stdClass();
			$memberInfo->member_srl = $args->member_srl = $logged_info->member_srl;
			$output = executeQuery('member.getDefaultDriver', $args);
		 ...
		 * @access public
		 * @param $memberInfo
		 * @param $password_is_hashed
		 * @param $driver
		 * @return Object
		 * @developer NHN (developers@xpressengine.com)
		 **/
		public function insertMember(&$memberInfo, $password_is_hashed = FALSE, $driver = 'default')
		{
			$args = $memberInfo;
			// Call a trigger (before)
			$output = ModuleHandler::triggerCall('member.insertMember', 'before', $memberInfo);
			if(!$output->toBool())
			{
				return $output;
			}
        ...
        function procMemberLeave(){
            ...
            // Get information of logged-in user
            $logged_info = Context::get('logged_info');
			$args->member_srl = $member_srl = $logged_info->member_srl;
			...
			$output = $oDriver->isValidateUnregister($args);
			...
			$output = $this->deleteMember($member_srl, $driver);
         ...
         **/
        function updateMember($args, $driver = 'default')
		{
            // Call a trigger (before)
			...
			if(!$this->memberInfo)
			{
				...
				$this->memberInfo = $oMemberModel->getMemberInfoByMemberSrl($args->member_srl, $site_module_info->site_srl, $columnList);
            }
			...
			{
				$commonArgs->allow_message = 'Y';
				unset($args->allow_message);
			}
         ...
         * @brief Delete User
         **/
        function deleteMember($member_srl, $driver = 'default')
		{
            // Call a trigger (before)
			...
			$output = $oDriver->deleteMember($member_srl);
			if(!$output->toBool())
			{
                $oDB->rollback();
                return $output;
