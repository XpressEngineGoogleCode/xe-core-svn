...
require_once _XE_PATH_ . 'modules/member/drivers/facebook/MemberVoFacebook.php';
require_once _XE_PATH_ . 'modules/member/drivers/facebook/FacebookApi.php';

/**
 * @brief class of facebook driver
 * @developer Arnia (support@xpressengine.org)
 */
class MemberDriverFacebook extends MemberDriver
{
	const SESSION_NAME = '__FACEBOOK_DRIVER__';
	private static $extractVars = array('facebookId', 'facebookNickName', 'face');
	...
	public function __construct()
	{
		...
		$this->oFacebookApi = new FacebookApi($config->appId, $config->appSecret);
	}
	...
	public function getMemberVo($memberSrl)
	{
		...
		$output = executeQuery('member.driver.facebook.getMemberInfoByMemberSrl', $args);
		if(!$output->toBool())
		{
		...
		$memberVo = new MemberVoFacebook($output->data);

		return $memberVo;
	}

	/**
	 * @brief Get MemberVo By facebook id
	 * @access public
	 * @param $facebookId
	 * @return MemberVo
	 * @developer Arnia (support@xpressengine.org)
	 */
	public function getMemberVoByFacebookId($facebookId)
	{
		$args->user_id = $facebookId;
		$output = executeQuery('member.driver.facebook.getMemberInfo', $args);
		if(!$output->toBool())
		{
			throw new MemberDriverException($output->getMessage());
		}
	...
	public function getMemberInfoWithSignupForm($memberSrl)
	{
		...
		$formList = array('facebookId', 'facebookNickName', 'face');
		$langList = array('facebook_id', 'facebook_nick_name', 'facebook_face');
	...
	{
		if(!isset($memberInfo->facebookId, $memberInfo->facebookNickName, $memberInfo->face))
		{
			return new Object(-1, 'facebook_msg_missing_facebook_id');
		}

		// check duplicate
		$memberVo = $this->getMemberVoByFacebookId($memberInfo->facebookId);
		if($memberVo)
		{
		...
		$args->user_id = $memberInfo->facebookId;
		$args->nick_name = $memberInfo->facebookNickName;
		...
		$output = executeQuery('member.driver.facebook.insertMember', $args);
		if(!$output->toBool())
		{
			$oDB->rollback();
			return $output;
	...
	public function deleteMember($memberSrl)
	{
		...
		$args->member_srl = $memberSrl;
		$output = executeQuery('member.driver.facebook.deleteMember', $args);
		return $output;
	}
	...
	public function updateMember($memberInfo)
	{
		...
		$output = executeQuery('member.driver.facebook.updateMember', $args);
		if(!$output->toBool())
		{
			return $output;
		}
	...
	public function isValidateSignUp($args)
	{
		...
		try
		{
			$person = $this->oFacebookApi->getPerson();
			$args->facebookId = $person->id;
			$args->facebookNickName = $person->nickname;
	...
	public function getSignupFormInfo($memberInfo = NULL)
	{
		...
		$memberInfo->facebookId = $person->id;
		$memberInfo->facebookNickName = $person->nickname;
	...
	public function getModifyFormInfo($memberSrl)
	{
		...
		$formInfo = new stdClass();
		$formInfo->title = '';
		$formInfo->inputTag = sprintf('<button id="refresh_info" type="button">%s</button>', Context::getLang('facebook_refresh_info'));
		$formInfo->description = Context::getLang('facebook_about_refresh_info');
	...
	private function getList()
	{
		...
		if($selectedGroupSrl)
		{
			$queryId = 'member.driver.facebook.getMemberListWithGroup';
		}
		else
		{
			$queryId = 'member.driver.facebook.getMemberList';
		}
	...
	/**
	 * @breif display login. redirect to facebook
	 * @access public
	...
	public function dispLogin($oModule)
	{
		...
		{
			return new Object(-1, 'facebook_msg_not_enough_api_info');
		}
		try
		{
			$this->oFacebookApi->doLogin();
		}
		catch(Exception $e)
		{
			return new Object(-1, $e->getMessage());
	...
	public function dispCallback($oModule)
	{
		...
		if($this->oFacebookApi->isLogged())
		{
			$userKey = $this->oFacebookApi->getUserKey();
			try
			{
				$memberVo = $this->getMemberVoByFacebookId($userKey->user_id);
			}
			...
			{
				// check id
				if($_SESSION[self::SESSION_NAME]['FACEBOOK_ID'] != $userKey->user_id)
				{
				...
				$this->oFacebookApi->destroySession();
				$this->destroySession();

				// unregister
				$output = $oMemberController->deleteMember($memberVo->getMemberSrl(), 'facebook');
				if(!$output->toBool())
				{
					return $output;
				}
			...
			if(!$memberVo)
			{
				...
				if($isExtendForm)
				{
					$url = getNotEncodedUrl('', 'vid', $vid, 'mid', $mid, 'document_srl', $documentSrl, 'act', 'dispMemberDriverInterface', 'driver', 'facebook', 'dact', 'dispSignUp');
					$oModule->setRedirectUrl($url);
					return new Object();
				}
				else
			...
			else
			{
				...
				if($person->nickname != $memberVo->getFacebookNickName() || $person->face != $memberVo->getFace())
				{
					// update information
					$args = new stdClass();
					$args->member_srl = $memberSrl;
				...
				// signin
				$output = $oMemberController->doSignin('facebook', $memberSrl);
				if(!$output->toBool())
				{
					return $output;
				}
	...
	{
		$config->appId = Context::get('appId');
		$config->appSecret = Context::get('appSecret');

		$oModuleController = getController('module');
		$output = $oModuleController->updateDriverConfig('member', 'facebook', $config);
		if(!$output->toBool())
		{
			return $output;
		}

		$oModule->setRedirectUrl(getNotEncodedUrl('', 'module', 'admin', 'act', 'dispMemberAdminDriverConfig', 'driver', 'facebook'));
	...
	public function procRefreshInfo($oModule)
	{
		...
		// check change of nick name or face url
		if($person->nickname != $oMemberVo->getFacebookNickName() || $person->face != $oMemberVo->getFace())
		{
			// update information
			$args = new stdClass();
	...
	public function procUnregister($oModule)
	{
		...
		$_SESSION[self::SESSION_NAME]['FACEBOOK_ID'] = $loggedInfo->facebookId;
	...
	public function getConfig()
	{
		...
		$config = $oModuleModel->getDriverConfig('member', 'facebook');
	...
	public function getSignUpFormInfoFromDB()
	{
		// get extend item
		$oMemberModel = getModel('member');
		$extendItems = $oMemberModel->getJoinFormListByDriver('facebook');
		if(!$extendItems)
		{
			return NULL;
		}
...
/* End of file MemberDriverFacebook.php */
/* Location: ./modules/member/drivers/facebook/MemberDriverFacebook.php */
