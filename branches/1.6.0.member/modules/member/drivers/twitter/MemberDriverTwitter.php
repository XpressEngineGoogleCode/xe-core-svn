...
require_once _XE_PATH_ . 'modules/member/drivers/twitter/MemberVoTwitter.php';
require_once _XE_PATH_ . 'modules/member/drivers/twitter/TwitterApi.php';

/**
 * @brief class of twitter driver
 * @developer Arnia (support@xpressengine.org)
 */
class MemberDriverTwitter extends MemberDriver
{
	const SESSION_NAME = '__TWITTER_DRIVER__';
	private static $extractVars = array('twitterId', 'twitterNickName', 'face');
	...
	public function __construct()
	{
		...
		$this->oTwitterApi = new TwitterApi($config->consumerKey, $config->consumerSecret);
	}
	...
	public function getMemberVo($memberSrl)
	{
		...
		$output = executeQuery('member.driver.twitter.getMemberInfoByMemberSrl', $args);
		if(!$output->toBool())
		{
		...
		$memberVo = new MemberVoTwitter($output->data);

		return $memberVo;
	}

	/**
	 * @brief Get MemberVo By twitter id
	 * @access public
	 * @param $twitterId
	 * @return MemberVo
	 * @developer Arnia (support@xpressengine.org)
	 */
	public function getMemberVoByTwitterId($twitterId)
	{
		$args->user_id = $twitterId;
		$output = executeQuery('member.driver.twitter.getMemberInfo', $args);
		if(!$output->toBool())
		{
			throw new MemberDriverException($output->getMessage());
		}
	...
	public function getMemberInfoWithSignupForm($memberSrl)
	{
		...
		$formList = array('twitterId', 'twitterNickName', 'face');
		$langList = array('twitter_id', 'twitter_nick_name', 'twitter_face');
	...
	{
		if(!isset($memberInfo->twitterId, $memberInfo->twitterNickName, $memberInfo->face))
		{
			return new Object(-1, 'twitter_msg_missing_twitter_id');
		}

		// check duplicate
		$memberVo = $this->getMemberVoByTwitterId($memberInfo->twitterId);
		if($memberVo)
		{
		...
		$args->user_id = $memberInfo->twitterId;
		$args->nick_name = $memberInfo->twitterNickName;
		...
		$output = executeQuery('member.driver.twitter.insertMember', $args);
		if(!$output->toBool())
		{
			$oDB->rollback();
			return $output;
	...
	public function deleteMember($memberSrl)
	{
		...
		$args->member_srl = $memberSrl;
		$output = executeQuery('member.driver.twitter.deleteMember', $args);
		return $output;
	}
	...
	public function updateMember($memberInfo)
	{
		...
		$output = executeQuery('member.driver.twitter.updateMember', $args);
		if(!$output->toBool())
		{
			return $output;
		}
	...
	public function isValidateSignUp($args)
	{
		...
		try
		{
			$person = $this->oTwitterApi->getPerson();
			$args->twitterId = $person->id;
			$args->twitterNickName = $person->nickname;
	...
	public function getSignupFormInfo($memberInfo = NULL)
	{
		...
		$memberInfo->twitterId = $person->id;
		$memberInfo->twitterNickName = $person->nickname;
	...
	public function getModifyFormInfo($memberSrl)
	{
		...
		$formInfo = new stdClass();
		$formInfo->title = '';
		$formInfo->inputTag = sprintf('<button id="refresh_info" type="button">%s</button>', Context::getLang('twitter_refresh_info'));
		$formInfo->description = Context::getLang('twitter_about_refresh_info');
	...
	private function getList()
	{
		...
		if($selectedGroupSrl)
		{
			$queryId = 'member.driver.twitter.getMemberListWithGroup';
		}
		else
		{
			$queryId = 'member.driver.twitter.getMemberList';
		}
	...
	/**
	 * @breif display login. redirect to twitter
	 * @access public
	...
	public function dispLogin($oModule)
	{
		...
		{
			return new Object(-1, 'twitter_msg_not_enough_api_info');
		}
		try
		{
			$this->oTwitterApi->doLogin();
		}
		catch(Exception $e)
		{
			return new Object(-1, $e->getMessage());
	...
	public function dispCallback($oModule)
	{
		...
		if($this->oTwitterApi->isLogged())
		{
			$userKey = $this->oTwitterApi->getUserKey();
			try
			{
				$memberVo = $this->getMemberVoByTwitterId($userKey->user_id);
			}
			...
			{
				// check id
				if($_SESSION[self::SESSION_NAME]['TWITTER_ID'] != $userKey->user_id)
				{
				...
				$this->oTwitterApi->destroySession();
				$this->destroySession();

				// unregister
				$output = $oMemberController->deleteMember($memberVo->getMemberSrl(), 'twitter');
				if(!$output->toBool())
				{
					return $output;
				}
			...
			if(!$memberVo)
			{
				...
				if($isExtendForm)
				{
					$url = getNotEncodedUrl('', 'vid', $vid, 'mid', $mid, 'document_srl', $documentSrl, 'act', 'dispMemberDriverInterface', 'driver', 'twitter', 'dact', 'dispSignUp');
					$oModule->setRedirectUrl($url);
					return new Object();
				}
				else
			...
			else
			{
				...
				if($person->nickname != $memberVo->getTwitterNickName() || $person->face != $memberVo->getFace())
				{
					// update information
					$args = new stdClass();
					$args->member_srl = $memberSrl;
				...
				// signin
				$output = $oMemberController->doSignin('twitter', $memberSrl);
				if(!$output->toBool())
				{
					return $output;
				}
	...
	{
		$config->consumerKey = Context::get('consumerKey');
		$config->consumerSecret = Context::get('consumerSecret');

		$oModuleController = getController('module');
		$output = $oModuleController->updateDriverConfig('member', 'twitter', $config);
		if(!$output->toBool())
		{
			return $output;
		}

		$oModule->setRedirectUrl(getNotEncodedUrl('', 'module', 'admin', 'act', 'dispMemberAdminDriverConfig', 'driver', 'twitter'));
	...
	public function procRefreshInfo($oModule)
	{
		...
		// check change of nick name or face url
		if($person->nickname != $oMemberVo->getTwitterNickName() || $person->face != $oMemberVo->getFace())
		{
			// update information
			$args = new stdClass();
	...
	public function procUnregister($oModule)
	{
		...
		$_SESSION[self::SESSION_NAME]['TWITTER_ID'] = $loggedInfo->twitterId;
	...
	public function getConfig()
	{
		...
		$config = $oModuleModel->getDriverConfig('member', 'twitter');
	...
	public function getSignUpFormInfoFromDB()
	{
		// get extend item
		$oMemberModel = getModel('member');
		$extendItems = $oMemberModel->getJoinFormListByDriver('twitter');
		if(!$extendItems)
		{
			return NULL;
		}
...
/* End of file MemberDriverTwitter.php */
/* Location: ./modules/member/drivers/twitter/MemberDriverTwitter.php */
