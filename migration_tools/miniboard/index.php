<?php 
    ...
    if($step == 2) {
        // 모듈 목록을 구해옴
        $query = sprintf("select * from %s_admin_board", $db_info->db_prefix);
        $module_list_result = $oMigration->query($query);
        while($module_info = $oMigration->fetch($module_list_result)) {
            $module_list[$module_info->id] = $module_info;
        }
    ...
    if($target_module) {
        if($target_module == 'module' && !$module_id) {
            ...
            switch($target_module) {
                case 'member' :
                        $query = sprintf("select count(*) as count from %s_member", $db_info->db_prefix);
                    break;
                case 'message' :
                        $query = sprintf("select count(*) as count from %s_memo", $db_info->db_prefix);
                    break;
                case 'module' :
                        $query = sprintf("select count(*) as count from %s_board_%s", $db_info->db_prefix, $module_id);
                    break;
            }
...
<body>
    ...
    <h1>miniboard data export tool ver 0.2</h1>
    ...
    <input type="hidden" name="path" value="<?php echo $path?>" />
        ...
        <blockquote>miniboard는 회원정보와 그외 모듈 종류를 나누어 추출하실 수 있습니다.</blockquote>
    ...
    <input type="hidden" name="module_id" value="<?php echo $module_id?>" />
        ...
        <blockquote>
            ...
            다운을 받지 않고 URL을 직접 miniboard 데이터이전 모듈에 입력하여 데이터 이전하실 수도 있습니다.
        </blockquote>
