<?php
    ...
    if($step == 2) {
        $query = "select Rid as module_srl, Topic as browser_title from T_Topics";
        $module_list_result = $oMigration->query($query);
        while($module_info = $oMigration->fetch($module_list_result)) {
            if(!$module_info->browser_title) continue;
    ...
    if($target_module) {
        if($target_module == 'module' && !$module_id) {
            ...
            switch($target_module) {
                case 'member' :
                        $query = sprintf("select count(*) as count from T_Users");
                    break;
                case 'module' :
                        $query = sprintf("select count(*) as count from T_Stories where Topic = '%d'", $module_id);
                    break;
            }
...
<body>
    ...
    <h1>korweblog data export tool ver 0.2</h1>
    ...
    <form action="./index.php" method="post">
        ...
        <ul>
            <li>
                korweblog 가 설치된 경로를 입력해주세요.

                <blockquote>
                예1) /home/아이디/public_html/korweblog<br />
                예2) ../korweblog
                </blockquote>
    ...
    <input type="hidden" name="path" value="<?php echo $path?>" />
        ...
        <blockquote>korweblog는 회원정보와 그외 모듈 종류를 나누어 추출하실 수 있습니다.</blockquote>
            ...
            <li>
                <label for="module">
                    ...
                    <?php
                        ...
                        $title = iconv('EUC-KR','UTF-8',$module_info->browser_title);
                    ?>
